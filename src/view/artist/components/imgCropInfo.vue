  ...
  methods: {
    ...
    initializeCropper() {
      this.$nextTick(() => {
        ...
        if (imageElement) {
          ...
          // Tạo Cropper với tỷ lệ hình tròn (aspectRatio = 1:1)
          this.cropper = new Cropper(imageElement, {
            aspectRatio: 1,  // Đảm bảo khung cắt là hình tròn
            viewMode: 1,
            dragMode: "move",
            cropBoxResizable: true,
            cropBoxMovable: true,
            ready: () => {
              // Đảm bảo crop box là hình tròn
              const cropBox = this.cropper.cropBox;
              cropBox.style.borderRadius = '50%';  // Đảm bảo crop box hình tròn
              cropBox.style.border = "3px solid #f44336";  // Tùy chỉnh viền
            }
          });
        }
      });
    ...
    cropImage() {
      if (this.cropper) {
        ...
        this.$emit("crop-complete", croppedImage);  // Đưa hình ảnh đã cắt ra ngoài component
        this.imageSrc = null;
        this.cropper.destroy();
      }
    },
